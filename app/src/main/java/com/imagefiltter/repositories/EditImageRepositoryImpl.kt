...
class EditImageRepositoryImpl(private val context: Context): EditImageRepository {
    override suspend fun prepareImagePreview(imageUri: Uri): Bitmap? {
        getInputStreamFromUri(imageUri)?.let { inputSteam ->
            val originalBitmap = BitmapFactory.decodeStream(inputSteam)
    ...
    override suspend fun getImageFilter(image: Bitmap): List<ImageFilter> {
...
//        endregion
    ...
    override suspend fun saveFilteredImage(filterBitmap: Bitmap): Uri? {
        return try {
